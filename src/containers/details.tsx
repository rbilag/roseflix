		...
		() => {
			if (detailsTrailer) {
				...
				if (category === 'series') {
					...
					trailerEndpoint = SECTIONS.series.helpers.fetchTVVideos.replace('{{tv_id}}', detailsTrailer.id);
				} else {
					...
					trailerEndpoint = SECTIONS.movies.helpers.fetchMovieVideos.replace('{{movie_id}}', detailsTrailer.id);
				}
				...
				if (!detailsTrailer.key) {
					movieHttp
						...
						.then((response) => {
							...
							if (trailerDetails)
								setDetailsTrailer((prevValue: DetailsTrailer) => ({ ...prevValue, key: trailerDetails.key }));
						})
						.catch((e) => console.log(e));
				}
			}
		},
		[ detailsTrailer, setDetailsTrailer, category ]
	);
